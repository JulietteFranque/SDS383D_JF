...
class Model:
    def __init__(self, X_groups, y_groups, total_observations, n_iter=1000):
        ...
        self.gamma = np.ones(self.P)
        self.betas = np.ones((self.P, len(X_groups)))
        self.n_iter = n_iter
        self.traces = {'betas': np.zeros([self.n_iter, self.P, len(X_groups)]), 'gamma': np.zeros((self.n_iter, self.P)), 'sigma_squared': np.zeros(self.n_iter), 'tau_squared': np.zeros(self.n_iter)}
    ...
    def _update_betas(self):
        for store_number in range(self.n_stores):
            K_inv = np.linalg.inv(1 / self.sigma_squared * self.X_groups[store_number].T @ self.X_groups[
                store_number] + 1 / self.tau_squared * np.identity(self.P))
    ...
    def _update_gamma(self):
        K_inv = np.linalg.inv(self.n_stores * np.identity(self.P) / (self.sigma_squared * self.tau_squared))
        m = K_inv @ np.sum(self.betas, axis=1)
