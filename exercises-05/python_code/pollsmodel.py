...
class Model:
    def __init__(self, X, y, n_iter=1000):
        ...
        self.S = len(self.X)
        self.n_iter = n_iter
        self.lower_bounds, self.upper_bounds = self._get_trunc_normal_params()
    ...
    def _update_z(self):
        self.z = []
        for state in range(self.S):
            self.z.append(truncnorm.rvs(self.lower_bounds[state], self.upper_bounds[state], loc=0, scale=1))
